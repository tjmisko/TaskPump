...
zero, but it is a *held* pause: the watchdog re-stamps it every poll and hands it
back when it exits, and one that nothing has refreshed for
`TASKPUMP_POOL_CAP_STALE_SEC` is expired and cleared. So a hand-written `0` is
not a pause that lasts either — `1` is still the number to write by hand.
