...
| `TASKPUMP_POOL_CAP_STALE_SEC` | `tp pump` (through `lib/pump-lib.sh`), `tp disk-watchdog` (to warn when its poll is slower than the window) | `900` | How long a cap file holding **`0`** stays believable without being refreshed. The watchdog re-stamps its pause every poll and hands it back when it exits; a `0` that has gone this long unrefreshed therefore outlived whatever wrote it, and the reader ignores it (naming the file and the age on stderr) and clears the file to the cap actually in force. `0` disables the expiry. A positive cap never expires — that is a standing preference, not a pause. |
