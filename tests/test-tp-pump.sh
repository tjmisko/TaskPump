...
echo "--- Test 33: a stranded claim as the range's last work is never DRAINED (#48) ---"
...
# 33a: the exit code. A foreign-branch claim as the only remaining work must
...
# 33b: the state file an operator reads during the page. `open_tasks: 0` with
...
# 33c: the plan has to say the same thing the exit code does. Filing the phase
...
# 33d: the issue's literal repro — claimed at phase grain, run at task grain.
...
# 33e: the guard is about claims nobody is driving, not about claims. A LIVE
...
# 33f: and a range that genuinely finished still drains. The guard must cost the
